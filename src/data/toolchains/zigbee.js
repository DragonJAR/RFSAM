export default {
  ...
  "facts": [
    ...
    { "k": "Security", "v": "AES-128-CCM* at NWK and APS layers. A network key shared by the whole PAN; a Trust Center link key gates joining. Default well-known TC link key 'ZigBeeAlliance09' (hex 5A6967426565416C6C69616E63653039)" },
  ...
  "layers": {
    ...
    "SP": {
      ...
      "tools": [
        { "tool": "killerbee", "role": "Channel / network scan", "why": "zbstumbler walks the channels transmitting beacon requests and logs the PANs that answer (PAN ID, channel, stack profile) — an active scan that finds the network and its channel fast.", "caveat": "Active: it transmits beacon requests. For a purely passive sweep, energy-scan with the capture radio instead. Runs on its own KillerBee radios (ApiMote / RZUSBstick / nRF52840) — not on the CatSniffer.", "deps": ["apimote", "nrf52840-dongle"] },
        ...
        { "tool": "kismet", "role": "Passive channel survey", "why": "Kismet channel-hops the 802.15.4 band and logs every PAN, coordinator and device it hears to pcapng with GPS — a passive survey that, unlike zbstumbler, never sends a beacon request. It drives the widest set of 802.15.4 radios here (CatSniffer v3, nRF52840, CC2531, RZUSBstick), so you survey with whatever board you carry.", "deps": ["catsniffer", "nrf52840-dongle", "cc2531"], "caveat": "Passive only. The CatSniffer Zigbee datasource needs a v3 (v1/v2 can’t capture Zigbee in Kismet) flashed with the RP2040 passthrough + Zigbee firmware, on Kismet git / a 2025-10+ release; it appears as a serial port (specify it manually)." }
      ]
    },
    "PHY": {
    ...
    "LL": {
      "note": "Capture and decode 802.15.4 / Zigbee frames. Park an 802.15.4 radio on the target's channel and it pulls beacons, MAC frames, the join exchange and the encrypted NWK/APS payloads off the air into a PCAP. Pick a capture tool by the radio you have; each feeds Wireshark, whose Zigbee dissector decodes NWK and APS (and will decrypt the upper layers once you supply the network key). On a CatSniffer the sniffer software is catnip (Electronic Cats' CatSniffer-Tools, which bundles pycatsniffer) — it streams 802.15.4/Zigbee frames into Wireshark over its extcap. Crucially, capture a device joining — that is where the network key is transported and where the whole network opens up.",
      "decoder": "wireshark",
      "tools": [
        { "tool": "killerbee", "role": "Primary capture", "why": "zbdump writes a PCAP and zbwireshark pipes frames live into Wireshark, on whichever 802.15.4 radio KillerBee supports — the standard capture path for Zigbee assessment.", "caveat": "Runs on KillerBee's own radios (ApiMote / RZUSBstick / nRF52840), not the CatSniffer.", "deps": ["apimote", "nrf52840-dongle"] },
        ...
        { "tool": "whsniff", "role": "CC2531 capture", "why": "Bridges a TI CC2531 USB dongle to Wireshark for live 2.4 GHz 802.15.4 capture — a cheap, simple capture path when you have a CC2531.", "caveat": "CC2531 is 2.4 GHz only and capture-only (no injection).", "deps": ["cc2531"] },
        ...
        { "tool": "kismet", "role": "Multi-radio capture", "why": "Captures raw 802.15.4/Zigbee frames to pcapng (open them in Wireshark) while it surveys, and supports more 802.15.4 radios than anything else here — a dedicated CatSniffer v3 Zigbee datasource (channels 12–26), plus nRF52840, CC2531, RZUSBstick and NXP KW41Z — so one tool covers most boards you’ll meet.", "deps": ["catsniffer", "nrf52840-dongle", "cc2531"], "caveat": "CatSniffer must be a v3 with the RP2040 serial-passthrough + Zigbee capture firmware (v1/v2 can’t); needs Kismet git or a 2025-10+ release, and the source is specified manually as a serial port." }
      ]
    },
    "CR": {
      ...
      "tools": [
        { "tool": "zbdsniff", "role": "Network-key extraction", "why": "Scans a join capture for the APS Transport-Key and recovers the Zigbee network key — decrypting it under the well-known default Trust Center link key 'ZigBeeAlliance09' (or reading it directly when sent in the clear).", "caveat": "Only works when the key was transported under the default TC link key or unencrypted; a per-device install code defeats it.", "deps": ["wireshark"], "needs": "A PCAP that contains a device joining the network (the APS Transport-Key frame). Capture the join at the Capture step (KillerBee / catnip on a CatSniffer / nRF Sniffer / whsniff), save the PCAP from Wireshark, then run zbdsniff over that file. To force a fresh join, power-cycle or re-pair the target device while capturing." },
        { "tool": "wireshark", "role": "Decrypt with the key", "why": "Once you have the network key, paste it into Wireshark's Zigbee protocol preferences (and the well-known TC link key 'ZigBeeAlliance09') and the dissector decrypts NWK/APS in the capture — turning ciphertext into readable cluster commands.", "deps": [], "needs": "The network key recovered by zbdsniff (or a key you already hold). Add it under Edit → Preferences → Protocols → ZigBee, then reload the capture." }
      ]
    },
    "AT": {
      "note": "Actively take over or disrupt the mesh. Pick the technique; each names the 802.15.4 radio it runs on, and replay/injection need a radio that can transmit (the ApiMote, and the CatSniffer via catnip, can; capture-only dongles cannot). With the network key in hand you can forge and inject frames; even without it, captured encrypted commands can sometimes be replayed wholesale, and a forced leave/rejoin can be abused to re-capture a join (and thus the key).",
      "tools": [
        { "tool": "killerbee", "role": "Replay & inject", "why": "zbreplay re-transmits frames captured earlier (e.g. an on/off or unlock command) back into the network, and the framework's tools forge frames once you hold the network key — the practical replay/injection path.", "caveat": "Needs a transmit-capable KillerBee 802.15.4 radio (ApiMote); capture-only dongles can't inject.", "deps": ["apimote"] },
        { "tool": "killerbee", "role": "Join / rejoin abuse", "why": "Force a target to leave and rejoin so a fresh join (and its key transport) hits the air again — KillerBee's framework drives the disruptive scans/floods that provoke a rejoin, which you then capture for key extraction at the Break-crypto step.", "caveat": "Disruptive and noisy; transmits into the live network.", "deps": ["apimote"] },
        { "tool": "catnip", "role": "OTA firmware-update MITM / jamming", "why": "Electronic Cats' CatSniffer OTA PoC drives the CC1352 CatSniffer to sniff a Zigbee OTA firmware update and capture the APS Transport-Key (decrypting the exchange without the default key), then jams the legitimate OTA server to force the client to rejoin a rogue server and accept attacker-supplied firmware. See poc/Poc_Zigbee in github.com/ElectronicCats/CatSniffer (CatSniffer MITM ZigBee OTA Server-Client / CatSniffer Targeting OTA Server).", "caveat": "The jamming/rogue-server stage needs an external jammer alongside the CatSniffer (per the PoC); the sniff/MITM stage runs on the CatSniffer alone.", "deps": ["catsniffer"] },
    ...
    "AP": {
      ...
      "tools": [
        { "tool": "killerbee", "role": "Craft & inject ZCL", "why": "With the network key, use KillerBee's framework (zbscapy / scapy-radio) to build and transmit Zigbee APS/ZCL frames — issue cluster commands (on/off, lock/unlock, level) the target device executes.", "caveat": "Needs the network key (from the Break-crypto step) and a transmit-capable KillerBee radio (ApiMote).", "deps": ["apimote"], "needs": "The network key recovered at the Break-crypto step, plus a transmit-capable KillerBee 802.15.4 radio (ApiMote)." }
      ]
    }
  }
};
