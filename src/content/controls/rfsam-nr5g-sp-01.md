...
> [!FLAG] The exact FR1 ~410 MHz–7.125 GHz / FR2 ~24.25–52.6 GHz spans and the common-band list (n28/n1/n3/n78) are taken from the NR5G Wayfinder facts and are consistent with the TS 38.101 band tables; the precise band edges vary by region and 3GPP release and should be reconciled against the current TS 38.101 operating-band table for a given market before being quoted as authoritative.

> [!FLAG] The 5GReasoner ACM DOI page (dl.acm.org/doi/10.1145/3319535.3354263) returns HTTP 403 to automated fetching; the title, authors (Hussain, Echeverria, Karim, Chowdhury, Bertino) and venue (ACM CCS 2019) were confirmed from the ACM listing and the authors' own publication page, and the DOI is the canonical record. A reviewer should open it in a browser to confirm.
