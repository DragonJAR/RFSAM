...
> [!FLAG] The 900 s / 15-minute RPA-rotation value is sourced from the Bluetooth SIG blog [bt-rpa-timeout] and is widely reflected in vendor stacks; confirm the exact normative wording and any version change against the adopted Core Specification GAP text before treating "15 minutes" as a hard spec requirement rather than a recommended default.

> [!FLAG] The Givehchian IEEE S&P 2022 paper is cited here via the authors' official UC San Diego project page (resolves); the IEEE Xplore record (document 9833758) returns an anti-bot HTTP 418 to automated fetches but is the paper's primary publication — verify the Xplore DOI/record directly in a browser if a canonical IEEE URL is required.
